<%@page import="org.quickbundle.modules.quickquery.RmQuickQueryCache"%>
...
<%  //判断是否为修改页面
	...
	if(request.getParameter("quick_query_id") != null) {
		quickQuery = RmQuickQueryCache.getSingleton().getQuickQueryById(request.getParameter("quick_query_id"));
	} else if(resultVo != null) {
		quickQuery = RmQuickQueryCache.getSingleton().getQuickQueryById(resultVo.getQuick_query_id());
	}
%>
...
<table class="mainTable">
	...
	<tr <%=quickQuery != null && !"2".equals(quickQuery.getOld_resource_id_type()) ? "style='display:none'" : ""%>>
	...
	<tr <%=quickQuery != null && !"1".equals(quickQuery.getOld_resource_id_type()) ? "style='display:none'" : ""%>>
...
<%  //取出要修改的那条记录，并且回写表单
	if(isModify) {  //如果本页面是修改页面
		...
		out.print(RmVoHelper.writeBackMapToForm(RmVoHelper.getMapFromRequest(request, IRmQuickQueryDataConstants.DEFAULT_CONDITION_KEY_ARRAY)));  //输出表单回写方法的脚本
  	}
%>
