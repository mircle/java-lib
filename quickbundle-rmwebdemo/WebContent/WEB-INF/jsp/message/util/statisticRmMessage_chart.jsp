...
<%@page import="org.quickbundle.tools.support.report.jfreechart.WebChart"%>
<%@page import="org.quickbundle.project.RmProjectHelper"%>
<%@page import="org.quickbundle.modules.message.rmmessage.util.IRmMessageConstants"%>
<%
	WebChart chart = new WebChart();
	List<String[]> lResult = RmProjectHelper.getCommonServiceInstance().doQuery("select parent_message_id as rm_key, count(parent_message_id) as rm_count from RM_MESSAGE group by parent_message_id", new RowMapper() {
	...
	PieURLGenerator pug = new PieURLGenerator() {
		public String generateURL(PieDataset dataset, Comparable key, int pieIndex) {
			return contextPath + "/RmMessageConditionAction.do?cmd=queryAll&parent_message_id=" + key;
		}
	};
	//饼图2D
	String filename_pie = chart.generatePieChart("按" + IRmMessageConstants.TABLE_COLUMN_CHINESE.get("parent_message_id") + "统计", session, new PrintWriter(out), pug, false); //如最后一位参数是true，则为3D饼图
	...
	CategoryURLGenerator cug = new CategoryURLGenerator() {
	    public String generateURL(CategoryDataset dataset, int series, int category) {
	    	return contextPath + "/RmMessageConditionAction.do?cmd=queryAll&parent_message_id=" + dataset.getColumnKey(series);
	    }
	};
	//柱图
	String filename_bar = chart.generateBarChart("按" + IRmMessageConstants.TABLE_COLUMN_CHINESE.get("parent_message_id") + "统计", session, new PrintWriter(out), cug, true);
