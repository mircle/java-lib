...
<title><%=IRmMessageConstants.TABLE_NAME_CHINESE%></title>
...
Ext.onReady(function(){
			...
			items:[
				{
					columnWidth: .33,
					layout: 'form',
					items: [{xtype: 'textfield', fieldLabel: '<%=IRmMessageConstants.TABLE_COLUMN_CHINESE.get("biz_keyword")%>', name: 'biz_keyword', anchor:'95%'}]
				},
				{
					columnWidth: .33,
					layout: 'form',
					items: [{xtype: 'textfield', fieldLabel: '<%=IRmMessageConstants.TABLE_COLUMN_CHINESE.get("sender_id")%>', name: 'sender_id', anchor:'95%'}]
				},
				{
					columnWidth: .33,
					layout: 'form',
					items: [{
						xtype: 'referencefield', fieldLabel: '<%=IRmMessageConstants.TABLE_COLUMN_CHINESE.get("parent_message_id")%>', anchor:'95%', name: 'parent_message_id', 
						urlPath: '<%=request.getContextPath()%>/RmMessageAction.do?cmd=queryReference&referenceInputType=radio'
					}]
				}
            ]
        },
			...
			items:[
				{
					columnWidth: .33,
					layout: 'form',
					items: [{xtype: 'textfield', fieldLabel: '<%=IRmMessageConstants.TABLE_COLUMN_CHINESE.get("owner_org_id")%>', name: 'owner_org_id', anchor:'95%'}]
				},
				{
					columnWidth: .33,
					layout: 'form',
					items: [{xtype: 'textfield', fieldLabel: '<%=IRmMessageConstants.TABLE_COLUMN_CHINESE.get("template_id")%>', name: 'template_id', anchor:'95%'}]
				},
				{
					columnWidth: .33,
					layout: 'form',
					items: [{xtype: 'textfield', fieldLabel: '<%=IRmMessageConstants.TABLE_COLUMN_CHINESE.get("is_affix")%>', name: 'is_affix', anchor:'95%'}]
				}
            ]
        },
		{
			...
			items:[
				{
					columnWidth: .33,
					layout: 'form',
					items: [{xtype: 'textfield', fieldLabel: '<%=IRmMessageConstants.TABLE_COLUMN_CHINESE.get("record_id")%>', name: 'record_id', anchor:'95%'}]
				},
				{
					columnWidth: .33,
					layout: 'form',
					items: [{xtype: 'textfield', fieldLabel: '<%=IRmMessageConstants.TABLE_COLUMN_CHINESE.get("message_xml_context")%>', name: 'message_xml_context', anchor:'95%'}]
				}
            ]
        }
    ...
    var columnModel = new Ext.ux.grid.LockingColumnModel({
        ...
        columns: [
            ...
            //expander, //可点击展开的+号
            {
                header: '<%=IRmMessageConstants.TABLE_COLUMN_CHINESE.get("id")%>',
                dataIndex: 'id',
                width: 0 //主键默认隐藏
            },
			...
			{
                header: '<%=IRmMessageConstants.TABLE_COLUMN_CHINESE.get("biz_keyword")%>',
                dataIndex: 'biz_keyword',
                editor: new fm.TextField({ allowBlank: false })
			},
			{
                header: '<%=IRmMessageConstants.TABLE_COLUMN_CHINESE.get("sender_id")%>',
                dataIndex: 'sender_id',
                editor: new fm.TextField({ allowBlank: false })
			},
			{
                header: '<%=IRmMessageConstants.TABLE_COLUMN_CHINESE.get("parent_message_id")%>',
                dataIndex: 'parent_message_id',
				editor: new fm.ReferenceField({ urlPath: '<%=request.getContextPath()%>/RmMessageAction.do?cmd=queryReference&referenceInputType=radio' })
			},
			{
                header: '<%=IRmMessageConstants.TABLE_COLUMN_CHINESE.get("owner_org_id")%>',
                dataIndex: 'owner_org_id',
                editor: new fm.TextField({  })
			},
			{
                header: '<%=IRmMessageConstants.TABLE_COLUMN_CHINESE.get("template_id")%>',
                dataIndex: 'template_id',
                editor: new fm.TextField({  })
			},
			{
                header: '<%=IRmMessageConstants.TABLE_COLUMN_CHINESE.get("is_affix")%>',
                dataIndex: 'is_affix',
                editor: new fm.TextField({  })
			},
			{
                header: '<%=IRmMessageConstants.TABLE_COLUMN_CHINESE.get("record_id")%>',
                dataIndex: 'record_id',
                editor: new fm.TextField({  })
			},
			{
                header: '<%=IRmMessageConstants.TABLE_COLUMN_CHINESE.get("message_xml_context")%>',
	...
	var store = new Ext.data.JsonStore({
	    ...
	    proxy: new Ext.data.HttpProxy({
	        url: '<%=request.getContextPath()%>/RmMessageAjaxAction.do?cmd=simpleQuery'
	    }),
        remoteSort: true,
    ...
    var grid = new Ext.grid.EditorGridPanel({
        title: '<%=IRmMessageConstants.TABLE_NAME_CHINESE%>列表',
        store: store,
...
<%}%>
           			Ext.Ajax.request({
	           			url: '<%=request.getContextPath()%>/RmMessageAjaxAction.do?cmd=insertUpdateBatch', 
	           			params: params,
				...
				if (selModel.hasSelection()) {
					Ext.Msg.confirm("警告", "确定要删除吗？", function(button) {
						if (button == "yes") {
							...
							if(ids != '') {
			           			Ext.Ajax.request({
				           			url: '<%=request.getContextPath()%>/RmMessageAjaxAction.do?cmd=deleteMulti', 
				           			params: {ids: ids},
				           			success: function(response) {
