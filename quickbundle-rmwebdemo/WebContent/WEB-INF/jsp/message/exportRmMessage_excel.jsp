...
<%@ page import="org.quickbundle.modules.message.rmmessage.service.IRmMessageService" %>
<%
	IRmMessageService service = (IRmMessageService)RmBeanFactory.getBean(IRmMessageConstants.SERVICE_KEY);	
	...
	try {
		...
		if(sColumn.contains("biz_keyword")) {
			...
			wsheet.addCell(new Label(columnIndex ++, rowIndex+2, IRmMessageConstants.TABLE_COLUMN_CHINESE.get("biz_keyword")));
		}
		if(sColumn.contains("sender_id")) {
			wsheet.addCell(new Label(columnIndex, rowIndex, ""));
			wsheet.addCell(new Label(columnIndex, rowIndex+1, "sender_id"));
			wsheet.addCell(new Label(columnIndex ++, rowIndex+2, IRmMessageConstants.TABLE_COLUMN_CHINESE.get("sender_id")));
		}
		if(sColumn.contains("parent_message_id")) {
			wsheet.addCell(new Label(columnIndex, rowIndex, ""));
			wsheet.addCell(new Label(columnIndex, rowIndex+1, "parent_message_id"));
			wsheet.addCell(new Label(columnIndex ++, rowIndex+2, IRmMessageConstants.TABLE_COLUMN_CHINESE.get("parent_message_id")));
		}
		if(sColumn.contains("owner_org_id")) {
			wsheet.addCell(new Label(columnIndex, rowIndex, ""));
			wsheet.addCell(new Label(columnIndex, rowIndex+1, "owner_org_id"));
			wsheet.addCell(new Label(columnIndex ++, rowIndex+2, IRmMessageConstants.TABLE_COLUMN_CHINESE.get("owner_org_id")));
		}
		if(sColumn.contains("template_id")) {
			wsheet.addCell(new Label(columnIndex, rowIndex, ""));
			wsheet.addCell(new Label(columnIndex, rowIndex+1, "template_id"));
			wsheet.addCell(new Label(columnIndex ++, rowIndex+2, IRmMessageConstants.TABLE_COLUMN_CHINESE.get("template_id")));
		}
		if(sColumn.contains("is_affix")) {
			wsheet.addCell(new Label(columnIndex, rowIndex, ""));
			wsheet.addCell(new Label(columnIndex, rowIndex+1, "is_affix"));
			wsheet.addCell(new Label(columnIndex ++, rowIndex+2, IRmMessageConstants.TABLE_COLUMN_CHINESE.get("is_affix")));
		}
		if(sColumn.contains("record_id")) {
			wsheet.addCell(new Label(columnIndex, rowIndex, ""));
			wsheet.addCell(new Label(columnIndex, rowIndex+1, "record_id"));
			wsheet.addCell(new Label(columnIndex ++, rowIndex+2, IRmMessageConstants.TABLE_COLUMN_CHINESE.get("record_id")));
		}
		if(sColumn.contains("message_xml_context")) {
			wsheet.addCell(new Label(columnIndex, rowIndex, ""));
			wsheet.addCell(new Label(columnIndex, rowIndex+1, "message_xml_context"));
			wsheet.addCell(new Label(columnIndex ++, rowIndex+2, IRmMessageConstants.TABLE_COLUMN_CHINESE.get("message_xml_context")));
		}
