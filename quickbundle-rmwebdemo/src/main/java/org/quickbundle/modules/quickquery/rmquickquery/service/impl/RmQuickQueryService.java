//代码生成时,文件路径: E:/platform/myProject/qbrm/code/QbRmWebDemo2/src/main/java/org/quickbundle/modules/quickquery/rmquickquery/service/impl/RmQuickQueryService.java
 ...
 * 
 * 文件名称: org.quickbundle.modules.quickquery.rmquickquery.service.impl --> RmQuickQueryService.java
 * 
 * 功能描述:
 * 
...
package org.quickbundle.modules.quickquery.rmquickquery.service.impl;
...
import org.quickbundle.modules.quickquery.RmQuickQueryCache;
...
public class RmQuickQueryService extends RmService implements IRmQuickQueryService, IRmQuickQueryConstants {
    
    /**
     * dao 表示: 数据访问层的实例
     */
    private IRmQuickQueryDao dao = null;
     ...
     * @return
     */
    public IRmQuickQueryDao getDao() {
        return dao;
    }
     ...
     * @param dao
     */
    public void setDao(IRmQuickQueryDao dao) {
        this.dao = dao;
    }
	...
	private void flushCache() {
		//刷新本地缓存
		RmQuickQueryCache.getSingleton().flushCache(IRmCacheListener.RefreshType.COMMON.value());
		//通知兄弟集群节点
		RmCacheHandler.getSingleton().flushOtherNodes(RmQuickQueryCache.class, IRmCacheListener.RefreshType.COMMON.value());
     ...
     * @return 返回新生成的id数组
     */
    public String[] insert(RmQuickQueryVo[] vos) {
    ...
    public RmQuickQueryVo find(String id) {
		RmQuickQueryVo vo = getDao().find(id);
     ...
     * @return 成功更新的记录最终数量
     */
	public int update(RmQuickQueryVo[] vos) {
	 ...
	 */
	public int[] insertUpdateBatch(RmQuickQueryVo[] vos) {
		int[] sum_insert_update = new int[2];
		List<RmQuickQueryVo> lInsert = new ArrayList<RmQuickQueryVo>();
		List<RmQuickQueryVo> lUpdate = new ArrayList<RmQuickQueryVo>();
		...
		if(lInsert.size() > 0) {
			sum_insert_update[0] = insert(lInsert.toArray(new RmQuickQueryVo[0])).length;
		}
		if(lUpdate.size() > 0) {
			sum_insert_update[1] = update(lUpdate.toArray(new RmQuickQueryVo[0]));
		}
     ...
     * @return 查询到的VO列表
     */
    public List<RmQuickQueryVo> queryByCondition(String queryCondition, String orderStr) {
     ...
     * @return 查询到的VO列表
     */
    public List<RmQuickQueryVo> queryByCondition(String queryCondition, String orderStr, int startIndex, int size) {
    ...
    public List<RmQuickQueryVo> queryByCondition(String queryCondition, String orderStr, int startIndex, int size, boolean selectAllClumn) {
        List<RmQuickQueryVo> lResult = getDao().queryByCondition(queryCondition, orderStr, startIndex, size, selectAllClumn);
